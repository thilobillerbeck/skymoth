...
export async function storeRepostRecord(
  ...
  return await db.insert(schema.repost).values({
    userId: userId,
    tootId: tootId,
...
export async function deleteUser(user: any) {
  return await db.delete(schema.user).where(eq(schema.user.id, user.id))
    .then(() => {
      logSchedulerEvent(
        user.name,
        "---",
...
export async function getMastodonInstanceUsers() {
  return await db
    ...
    .leftJoin(schema.user, eq(schema.user.mastodonInstanceId, schema.mastodonInstance.id))
    .groupBy(schema.mastodonInstance.id);
}

export async function deleteMastodonInstance(instance: any) {
  return await db.delete(schema.mastodonInstance).where(eq(schema.mastodonInstance.id, instance.id))
    .then(() => {
      logSchedulerEvent(
        "SYSTEM",
        instance.url,
...
export async function persistBlueskyCreds(
  ...
  return await db.update(schema.user).set({
    updatedAt: new Date(),
    blueskyHandle: handle,
    blueskyToken: token,
    blueskyPDS: pds,
  }).where(eq(schema.user.id, userId)).returning();
}
...
export async function createMastodonInstance(
  ...
  return await db.insert(schema.mastodonInstance).values({
    updatedAt: new Date(),
    url: instanceDomain,
