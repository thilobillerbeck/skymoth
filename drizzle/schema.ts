...
export const statusVisibility = pgEnum("StatusVisibility", [
	"public",
	"unlisted",
	"private",
	"direct",
...
export const mastodonInstance = pgTable(
	"MastodonInstance",
	{
		id: text()
			.primaryKey()
	...
	(table) => [
		uniqueIndex("MastodonInstance_urlEncoded_key").using(
			"btree",
			table.urlEncoded.asc().nullsLast().op("text_ops"),
		),
		uniqueIndex("MastodonInstance_url_key").using(
			"btree",
...
export const user = pgTable(
	"User",
	{
		...
		lastTootTime: timestamp({ precision: 3, mode: "date" })
			.defaultNow()
			.notNull(),
		name: text().notNull(),
